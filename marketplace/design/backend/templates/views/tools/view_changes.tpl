...
{capture name="mainbox"}
    ...
    <form action="{""|fn_url}" method="post" name="data_compare_form" enctype="multipart/form-data" class="form-horizontal form-edit">
        ...
        {capture name="buttons"}
            {if !$dist_filename}
                <a class="btn" href="{"tools.create_snapshot"|fn_url}">{__("jp_make_a_fresh_snapshot")}</a>
            {/if}
    ...
    <pre style="height: 300px; overflow-y: scroll" class="diff-container">{$db_d_diff nofilter}</pre>
        ...
        <div style="margin: 30px 20px 20px 7px;">
            <table cellpadding="0" cellspacing="0" border="0" width="30%">
                <tr>
                    <td><span class="label label-success">{__("added")}</span></td>
                    <td><span class="label label-warning">{__("change")}</span></td>
                    <td><span class="label label-important">{__("deleted")}</span></td>
                </tr>
            </table>
        </div>
        {/if}
