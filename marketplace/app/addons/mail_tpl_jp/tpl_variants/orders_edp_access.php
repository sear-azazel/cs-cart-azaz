...
// $Id: orders_edp_access.php by tommy from cs-cart.jp 2015
...
if( !empty($tpl_order_info) ){
	...
	foreach($tpl_order_info['products'] as $oi){
		...
		if( $oi['extra']['is_edp'] == 'Y' && $tpl_edp[$oi['product_id']]['files'] ){
			...
			$blk_edp .= $oi['product'] . "\n";
			$blk_edp .= $tpl_config['http_location'] . '/' . $tpl_config['customer_index'] . "?dispatch=orders.downloads&product_id=" . $oi['product_id'] . "&ekey=" . $first_file['ekey'];
			$blk_edp .= "\n\n";

			foreach( $tpl_edp[$oi['product_id']]['files'] as $file_id => $file ){
				$blk_edp .= $file['file_name'] . " (" . number_format($file['file_size'],0,'.',',') . __('bytes') . ")\n";
				$blk_edp .= $tpl_config['http_location'] . '/' . $tpl_config['customer_index'] . "?dispatch=orders.get_file&file_id=" . $file['file_id'] . "&product_id=" . $oi['product_id'] . "&ekey=" . $file['ekey'];
