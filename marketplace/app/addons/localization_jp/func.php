...
function fn_localization_jp_get_order_info(&$order, &$additional_data)
{
    ...
    // 注文確認書の出力の場合
    if( Registry::get('runtime.controller') == 'orders' && Registry::get('runtime.mode') == 'print_invoice') {
        // 注文割引が適用されている場合
        if( !empty($order['subtotal_discount']) ){
            // ポイントを利用した場合
            if( Registry::get('addons.reward_points') && Registry::get('addons.reward_points.status') == 'A' ){
                // 注文割引金額から利用ポイント金額をマイナス
                if (!empty($additional_data[POINTS_IN_USE])) {
                    $order['points_info']['in_use'] = unserialize($additional_data[POINTS_IN_USE]);
                    $order['subtotal_discount'] = $order['subtotal_discount'] - $order['points_info']['in_use']['cost'];
                }
            }
        }
    }
...
function fn_localization_jp_get_statuses_post(&$statuses, &$join, &$condition, &$type, &$status_to_select, &$additional_statuses, &$exclude_parent, &$lang_code, &$company_id)
{
    // 管理画面で注文ステータスを表示する場合
...
function fn_localization_jp_get_profile_fields(&$location, &$select, &$condition)
{
    ...
    if( $location == 'I' && ($_REQUEST['dispatch'] == 'orders.print_invoice' || $_REQUEST['dispatch'] == 'orders.bulk_print') ){
        ...
        $kana_fields = array(36,37,38,39,40,41);
        $condition .= db_quote(' AND ?:profile_fields.field_id NOT IN (?a)', $kana_fields);
    // マーケットプレイスへの参加申請ページまたは管理画面からの出品者登録において氏名ふりがなフィールドを取得しない
...
function fn_localization_jp_gather_additional_product_data_post(&$product, &$auth, &$params)
{
    ...
    if( !empty($product['tax_ids']) ){
        ...
        if( !empty($product_taxes) ){
            $calculated_data = fn_calculate_tax_rates($product_taxes, $tx_list_price, 1, $auth, $_SESSION['cart']);
            ...
            $calculated_data = fn_calculate_tax_rates($product_taxes, $tx_original_price, 1, $auth, $_SESSION['cart']);
            // Apply taxes to product subtotal
            if (!empty($calculated_data)) {
                foreach ($calculated_data as $_k => $v) {
                    $tax_value += $v['tax_subtotal'];
...
// Qdmailを使ってメール送信
function fn_lcjp_send_by_qdmail($mailer_org, $mail_tpl, $area, $is_html, $to, $attachments)
{
    // 送信するメールテンプレートが会員登録に関するものである場合
    if( $mail_tpl == 'profiles/create_profile.tpl' ){
        // クイック登録のオン/オフを取得
    ...
    // Qdmailライブラリを読み込み
    require_once(Registry::get('config.dir.addons') . 'localization_jp/lib/qdmail/qdmail.php');

    $mail = new Qdmail();

    // 日本語以外のメールは文字セットをUTF-8に
    if(CART_LANGUAGE != 'ja'){
    ...
    // CS-Cartで指定したメールの送信方法（PHPのmail関数 / SMTP / sendmal）を取得
    $send_method = $mailer_org->Mailer;

    // SMTPを使ってメールを送信する場合
    if( $send_method == 'smtp' ){

        // Qdsmtpライブラリを読み込み
        require_once(Registry::get('config.dir.addons') . 'localization_jp/lib/qdmail/qdsmtp.php');

        // QdmailでのSMTP送信を指定
        $mail->smtp(true);

        // SMTP認証を行う場合のポート番号
        $lcjp_adoon_setting = Registry::get('addons.localization_jp');
        if( $lcjp_adoon_setting['jp_smtp_port'] != 25 && $lcjp_adoon_setting['jp_smtp_port'] != 587 ){
            $lcjp_adoon_setting['jp_smtp_port'] = 587;
        }

        // SMTP認証の有無に応じてポート番号とプロトコルを設定
        if( $mailer_org->SMTPAuth ){
            $smtp_port = $lcjp_adoon_setting['jp_smtp_port'];
        ...
        $smtp_param = array(
            'host'=>$mailer_org->Host,		// メールサーバー
            'port'=>$smtp_port, 			// ポート番号
            'from'=>$mailer_org->From,		// Return-path:
            'protocol'=>$smtp_protocol,	// SMTPプロトコル
            'user'=>$mailer_org->Username,	// SMTPサーバーのユーザーID
            'pass'=>$mailer_org->Password,	// SMTPサーバーの認証パスワード
        );

        $mail->smtpServer($smtp_param);
    }

    // CS-Cart標準のメール送信用クラスから件名 / 送信者 / 送信者名 を取得
    $subject = $mailer_org->Subject;
    $from = $mailer_org->From;
    $from_name = $mailer_org->FromName;

    // Special Thanks to kit.t from CS-CartJP Users Forum
    // 送信者名にマルチバイト文字を含まない場合はクォーテーションで囲む
    // （送信者名にカンマなどが入るとメーラーで正しく表示されない不具合を解消）
    if( mb_strlen($from_name, 'UTF-8') == strlen($from_name) ){
        $from_name = '"' . $from_name . '"';
    }

    // 本文
    $view = Registry::get('view');
    $body = $view->displayMail($mail_tpl, false, $area);

    // フックポイントを設定
    $tmp_to = $to;
    fn_set_hook('before_setup_mail_contents', $subject, $mail_tpl, $body, $is_html, $tmp_to, $from, $from_name);

    // 件名
    $mail->subject($subject);

    // 添付ファイルおよびHTML中にインラインで表示する画像の処理
    $mail->Body = fn_lcjp_attach_images($body, $attachments, $mail, $is_html);
    ...
    // 送信先
    foreach($to as $v){
        $mail->to( trim($v), null , true );
    }

    // 複数のメールアドレスが存在する場合は個別アドレス分離機能を使用
    $mail->toSeparate( true );

    // 代替テキストの自動生成をオフ
    // （オンにすると注文確認メールなどの表示が崩れる）
    $mail->autoBoth(false);
    ...
    // メール送信に失敗した場合
    if( !$mail->send() ){
        // エラーが発生したら内容をメッセージボックスに表示
        $qd_err = $mail->errorStatment(false);
        fn_set_notification('E', __('error'), __('error_message_not_sent') . ' ' . $qd_err[2]);
        return false;

    // メール送信に成功した場合
    }else{
        // フックポイントを設定
        fn_set_hook('after_send_qdmail', $to, $subject, $mail_tpl, $body, $is_html);
        return true;
    }
}
...
function fn_lcjp_print_pdf_invoice($order_ids)
{
    ...
    foreach( $order_ids as $order_id ){
        ...
        if( !empty($order_data['notes']) ){
            // コメント欄行数
            $comment_line = $info_line_count;
            $display_notes = $order_data['notes'];
            // ポジションデータ初期化
            $_pos = 0;

            // サプライヤー対応
            // 管理者によりコメント欄を修正されていた場合、無限ループを防ぐため
            $_loop_limit = 10;
            // 有効な全てのサプライヤー名を取得
            $_suppliers = db_get_array("SELECT company FROM ?:companies WHERE status = 'A' AND lang_code = ?s", CART_LANGUAGE);
            $_suppliers[]['company'] = Registry::get('settings.Company.company_name');
            // コメントからサプライヤー名の位置を取得
            $_sname_pos = 1000;
            do {
                $_pos = mb_strpos($display_notes, '【', $_pos, 'UTF-8');
                if($_pos === false) break;
                // 最初に現れた【の後に有効なサプライヤー名が含まれればコメントと配送情報の区切りと見なす
                reset($_suppliers);
                foreach($_suppliers as $_supplier) {
                    $_check_string = mb_substr($display_notes, ($_pos + 1), mb_strlen($_supplier['company'], 'UTF-8'), 'UTF-8');
                    // mb_系関数より切り取った文字列の文字コードがASCIIになるため、文字コードが違う場合はあわせる
                    if(mb_detect_encoding($_check_string) != 'UTF-8') {
                        $_name = mb_convert_encoding($_supplier['company'], mb_detect_encoding($_check_string), 'UTF-8');
                    } else {
                        $_name = $_supplier['company'];
                    }
                    if($_name == $_check_string) {
                        $_sname_pos = $_pos;
                        break;
                    }
                }
                if($_sname_pos != 1000) break;
                $_pos++;
                $_loop_limit--;
                if($_loop_limit < 1) break;
            } while(1);
            // サプライヤー名がなかった場合お届け時間帯のみを切り取る
            if($_sname_pos == 1000) {
                $_sname_pos = mb_strpos($display_notes, __('jp_shipping_delivery_time') . " - ", 0, 'UTF-8');
                if($_sname_pos === false) {
                    $_sname_pos = mb_strpos($display_notes, __('jp_delivery_date') . " - ", 0, 'UTF-8');
                }
                // 配送情報がなかったばあいのポジション
                if($_sname_pos === false) $_sname_pos = 1000;
            }
            // お客様が入力したコメントから改行を削除
            $_real_note = str_replace("\r\n", '\n', mb_substr($display_notes, 0, $_sname_pos, 'UTF-8'));
            $_real_note = str_replace("\r", '\n', $_real_note);
            $_real_note = str_replace("\n", '', $_real_note);
            // メモからお届け情報を抽出
            $_supplier_text = mb_substr($display_notes, $_sname_pos, 1000, 'UTF-8');
            // お届け時間帯の - を ： に変換
            $_supplier_text = str_replace(__('jp_shipping_delivery_time') . " - ", __('jp_shipping_delivery_time') . __('jp_pdfinv_full_colon'), $_supplier_text);
            // 配送日の - を ： に変換
            $_supplier_text = str_replace(__('jp_delivery_date') . " - ", __('jp_delivery_date') . __('jp_pdfinv_full_colon'), $_supplier_text);
            // 支払方法
            // お客様コメントを印刷しない追加する場合は「 . "\n" . $_real_note」を追記
            $_memo_info = __('payment_methods') . __('jp_pdfinv_full_colon') . $order_data['payment_method']['payment'] . "\n" . $_supplier_text;
            // 支払方法
            // お届け情報（サプライヤー毎）
            // ショップコメント
            // お客様コメント
        ...
        if( $tax_total > 0 ){
            $pdf->SetX(120);
            $pdf->MultiCell(40, 0, __('jp_consumption_tax') . ' ' . number_format($tax['rate_value']) . '%', 1, 'R', 1, 0);
            $print_price = fn_lcjp_convert_price($tax_total, $order_currency, true);
            $pdf->MultiCell(45, 0, $print_price, 1, 'R', 0, 1);
        }
