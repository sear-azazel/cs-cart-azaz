...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if( $mode == 'check_email' ){
        ...
        if( isset($_REQUEST['email']) ){
            ...
            $user_id = db_get_field('SELECT user_id FROM ?:users WHERE email = ?s', $_REQUEST['email']);
    ...
    // 注文情報の作成の場合
        ...
        if( defined('AJAX_REQUEST') ){
            ...
            if( $client !== false || !isset($_REQUEST['orderReferenceId']) || empty($_REQUEST['orderReferenceId']) ){
                ...
                $_SESSION['amazon_order_reference_id'] = $_REQUEST['orderReferenceId'];

                // Amazonで管理している配送先住所情報をCS-Cartで利用可能な形に変換
                fn_prepare_amazon_shipping_address($response, $_SESSION['cart']['user_data']);
