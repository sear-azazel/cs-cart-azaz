...
// Modified for Japanese version by tommy from cs-cart.jp 2016
// YYYY/MM/DD 形式の日時指定に対応
...
function fn_discussion_parse_datetime($datetime)
{
    ...
    if (!empty($datetime)) {
        ...
        if (count($date_parts) == 3) {
            ...
            // YYYY/MM/DD 形式の日時指定に対応
            /////////////////////////////////////////////////////////////////////
            // POSTされた日付データ配列の最初の要素のデータ長が4バイトの場青
            if( strlen($date_parts[0]) == 4 ){
                // YYYY/MM/DD 形式と見なして処理
                $timestamp = mktime($h, $m, 0, $date_parts[1], $date_parts[2], $date_parts[0]);
            }elseif (Registry::get('settings.Appearance.calendar_date_format') == 'month_first') {
