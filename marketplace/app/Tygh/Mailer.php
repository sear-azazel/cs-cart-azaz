...
class Mailer extends \PHPMailer
{
    ...
    public static function sendMail($params, $area = AREA, $lang_code = CART_LANGUAGE)
    {
        ...
        ///////////////////////////////////////////////////////////////
        // Qdmailライブラリが存在し、メール用テンプレートが指定されている場合
        if (function_exists('fn_lcjp_send_by_qdmail') && !empty($params['tpl']) ) {
            // Qdmailライブラリを使用してメールを送信
            $is_html = isset($params['is_html']) ? $params['is_html'] : true;
            $result = fn_lcjp_send_by_qdmail($mailer, $params['tpl'], $area, $is_html, $to, !empty($params['attachments']) ? $params['attachments'] : '');
        // Qdmailライブラリが存在しない場合
        }else{
            // CS-Cartのデフォルト機能でメールを送信
            foreach ($to as $v) {
