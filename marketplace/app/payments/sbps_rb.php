...
// $Id: sbps_rb.php by tommy from cs-cart.jp 2015
// ソフトバンク・ペイメント・サービス（リンクタイプ・クレジットカード継続課金）
...
if (defined('PAYMENT_NOTIFICATION')) {
	...
	if ($mode == 'process') {
		...
		if( ($_REQUEST['merchant_id'] == $sbps_settings['processor_params']['merchant_id']) && ($_REQUEST['amount'] == round($sbps_order_info['total'])) && $_REQUEST['res_result'] == 'OK' && empty($_REQUEST['res_err_code']) ){
			if (fn_check_payment_script('sbps_rb.php', $_REQUEST['order_id'])) {
	...
	// 購入タイプ（「継続購入」で固定）
	$sbps_params['pay_type'] = 1;

    // 自動課金タイプ（「自動課金する」で固定）
    $sbps_params['auto_charge_type'] = 1;

	// サービスタイプ（「売上」で固定）
	$sbps_params['service_type'] = 0;

    // 決済区分（「前払い」で固定）
    $sbps_params['div_settele'] = 0;

    // キャンペーンタイプ（購入月無料か否か）
    $sbps_params['camp_type'] = $processor_data['processor_params']['camp_type'];

	// 顧客利用端末タイプ
	$sbps_params['terminal_type'] = 0;

	// 決済完了時URL
	$sbps_params['success_url'] = html_entity_decode(fn_url("payment_notification.process&payment=sbps_rb&order_id=$order_id", AREA, 'current'), ENT_QUOTES, 'UTF-8');

	// 決済キャンセル時URL
	$sbps_params['cancel_url'] = html_entity_decode(fn_url("payment_notification.cancelled&payment=sbps_rb&order_id=$order_id", AREA, 'current'), ENT_QUOTES, 'UTF-8');

	// エラー時URL
	$sbps_params['error_url'] = html_entity_decode(fn_url("payment_notification.error&payment=sbps_rb&order_id=$order_id", AREA, 'current'), ENT_QUOTES, 'UTF-8');
