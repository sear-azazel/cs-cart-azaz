...
$classLoader = require_once $config['dir']['root'] . '/app/lib/vendor/autoload.php';
$classLoader->add('Tygh', $config['dir']['root'] . '/app');
...
class RestoreDataKeeper extends DataKeeper
{
    ...
    {
        if (static::$archiver === null) {
            static::$archiver = new Archiver();
        }
    ...
    protected static function decompressFiles($backup_path, $extract_path)
    {
        ...
        try {
            ...
            $result = $archiver->extractTo($backup_path, $extract_path);
...
function db_connect($db_host, $db_user, $db_password, $db_name)
{
    if (class_exists('mysqli')) {
        ...
        printf("Required mysqli or pdo_mysql.");
        exit();
    }
}
