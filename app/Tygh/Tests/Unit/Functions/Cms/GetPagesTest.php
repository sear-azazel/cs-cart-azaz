    ...
    public function getPagesQueries()
    {
        return array(
            ...
            array(
                ...
                'query' => "SELECT pages.*, page_descriptions.* FROM pages LEFT JOIN page_descriptions ON pages.page_id = page_descriptions.page_id AND page_descriptions.lang_code = 'en' WHERE 1 AND pages.status IN ('D') AND pages.page_type IN ('T')   ORDER BY pages.parent_id asc, pages.position asc, page_descriptions.page asc "
            ),
        );
    }
}
