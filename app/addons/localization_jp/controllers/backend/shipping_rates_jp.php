		...
		if(!empty($default_carrier)) {
			...
			if(!empty($jp_carrier_rates)) {
				...
				if(!empty($_REQUEST['result_ids'])) {
					$file_content = trim(Registry::get('view')->display('addons/localization_jp/views/shipping_rates_jp/ajax_manage.tpl', false)); // FIX-TEMPLATE
					header("Content-type: text/html");
					header("Content-disposition: attachment; filename=shipping.html");
					echo $file_content;
					exit();
				}
			}
