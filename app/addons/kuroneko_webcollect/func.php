...
function fn_kuroneko_webcollect_get_orders(&$params, &$fields, &$sortings, &$condition, &$join, &$group)
{
    ...
    switch( Registry::get('runtime.controller') ){
        // クロネコwebコレクト（クレジットカード払い）
        case 'krnkwc_cc_manager':
            $processor_scripts = array('krnkwc_cc.php', 'krnkwc_ccreg.php');
            break;
...
function fn_kuroneko_webcollect_create_shipment(&$shipment_data, &$order_info, &$group_key, &$all_products)
{
    ...
    // 配送管理を行わない設定下において、追跡番号が入力されていない、またはクロネコヤマトに追跡番号を送信しない場合
    if( !$use_shipments && ( empty($shipment_data['send_slip_no']) || $shipment_data['send_slip_no'] != 'Y' ) ) {
        // do nothing
...
function fn_krnkwc_addon_uninstall()
{
    ...
    $file_list = array('krnkwc_cc.php', 'krnkwc_ccreg.php', 'krnkwc_cvs.php', 'krnkab.php');
...
function fn_krnkwc_get_params($type, $order_id, $order_info, $processor_data)
{
    ...
    switch($type){
        ...
        case 'credit':
            ...
            // 登録済みカード決済の場合
            }elseif( $processor_data['processor_script'] == 'krnkwc_ccreg.php' ){
...
function fn_krnkwc_get_card_code_api($card_no)
{
    ...
    // テスト用カード番号の場合
    if($twelve_digit == '000000000000' || $twelve_digit == '000000000001' || $twelve_digit == '000000000002' ){
        // VISAのコードを返す
        return 9;
    }
 ...
 * @return mixed|string
 */
function fn_krnkwc_send_request($type, $params, $mode)
{
 ...
 * @return bool
 */
function fn_krnkwc_add_shipment(&$shipment_data, $order_info)
{
...
function fn_krnkwc_get_registered_card_info($user_id, $is_delete_by_user = false)
{
    ...
    // 支払方法に関するデータを取得
    $payment_id = db_get_field("SELECT ?:payments.payment_id FROM ?:payments LEFT JOIN ?:payment_processors ON ?:payment_processors.processor_id = ?:payments.processor_id WHERE ?:payment_processors.processor_script = 'kuroneko_webcollect_ccreg.php' AND ?:payments.status = 'A'");
    $processor_data = fn_get_processor_data($payment_id);
...
function fn_krnkwc_cc_is_changeable($order_id, $processor_data)
{
    ...
    if( $parent_order_info['is_parent_order'] == 'N' && $parent_order_info['parent_order_id'] == 0 ) {
        ...
        $changable_processor_scripts = array('krnkwc_cc.php', 'krnkwc_ccreg.php');
