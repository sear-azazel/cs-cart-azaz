...
if( empty($_edit_mail_tpl) ) {
	...
	// 検索する注文のID
	$tpl_order_id = $tpl_base_data['o_id']->value;
	...
	if (!empty($tpl_config)) {
		if ($tpl_order_id) {
			...
			$blk_order_track .= $tpl_config['http_location'] . '/' . $tpl_config['customer_index'] . "?dispatch=orders.track&ekey=" . $tpl_base_data['access_key']->value . "&o_id=" . $tpl_order_id;
			$blk_order_track .= "\n\n";
		}

		$blk_order_track .= __('text_track_view_all_orders') . "\n";
		$blk_order_track .= $tpl_config['http_location'] . '/' . $tpl_config['customer_index'] . "?dispatch=orders.track&ekey=" . $tpl_base_data['access_key']->value;
	}
}else{
	$blk_order_track = '';
}
