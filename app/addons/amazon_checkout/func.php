...
function fn_amazon_checkout_get_payments($params, $fields, $join, $order, &$condition, $having)
{
    ...
    if (AREA == 'C') {
        // 「Amazonアカウントでお支払い」ページの場合
        if( fn_is_enabled_amazon_checkout() ){
            // 支払方法の抽出条件に「Amazonログイン&ペイメントであること」を追加
            $condition[] = db_quote('?:payments.is_amazon_payment = ?s', 'Y');
        // 通常の注文手続きページの場合
        }else{
            // 支払方法の抽出条件に「Amazonログイン&ペイメントでないこと」を追加
...
/**
 * ログイン済ユーザーがAmazonログイン＆ペイメントで決済する際に、アドレスウィジェットで選択した住所情報が無視され、
 ...
 * で $_SESSION['cart']['user_data'] の値が書き換えられるために発生する
...
function fn_amazon_checkout_get_user_info(&$user_id, &$get_profile, &$profile_id, &$user_data)
{
    ...
    // Amazonログイン＆ペイメントを利用して注文を行う場合
    if( AREA == 'C' && $_controller == 'checkout' && $_mode == 'checkout' && $_action == 'amazon_checkout' && !empty($_SESSION['cart']['user_data']['s_state']) ) {

        if( fn_is_enabled_amazon_checkout() && !empty($_SESSION['amazon_order_reference_id']) ){
            // Amazonログイン＆ペイメントのクライアントのインスタンスを作成
            $client = fn_get_client_exemplar();

            // Amazonログイン＆ペイメントのクライアントのインスタンスが存在する場合、Amazonで管理している配送先住所情報をCS-Cartで利用可能な形に変換
            if ($client !== false) {
...
function fn_amznlap_addon_before_install()
{
    ...
    // エラータイトルをセット
    $error_title = "「Amazonログイン&ペイメント」アドオン インストールエラー";
...
function fn_get_client_exemplar()
{
    ...
    // Amazonログイン＆ペイメントのクライアントクラス
    $class_name = '\Tygh\PayWithAmazon\Client';

    // Amazonログイン＆ペイメントのクライアントクラスが存在する場合
 ...
 * @return bool
 */
function fn_prepare_amazon_shipping_address($response, &$user_data)
{
...
/**
 * Amazonログイン&ペイメントによる支払いの有効/無効を判定
 *
 * @return bool
 */
function fn_is_enabled_amazon_checkout()
{
    return (!empty($_SESSION['show_amazon_checkout'])) ? true : false;
}
