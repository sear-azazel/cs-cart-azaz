...
// Amazonログイン＆ペイメントのクライアントのインスタンスを初期化
...
if( $client !== false || !isset($_SESSION['amazon_order_reference_id']) || empty($_SESSION['amazon_order_reference_id']) ){
    ...
    // 商品の配送先住所を更新
    if( !empty($_SESSION['order_shipping_info']) ){
        fn_update_order_shipping_info($order_info['order_id'], $_SESSION['order_shipping_info']);
    ...
    if( $client->success ){
        ...
        if( $pp_response['authorization_status'] == 'Declined' ){
            // エラーメッセージを表示してAmazonログイン＆ペイメント専用の注文手続きページに遷移
...
// Amazonログイン＆ペイメント用スクリプトを使って決済が行われた場合、注文確定処理を実施
