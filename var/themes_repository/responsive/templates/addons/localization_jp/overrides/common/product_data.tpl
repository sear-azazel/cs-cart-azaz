...
{* Modified to fix bug #006315 by tommy from cs-cart.jp 2016 *}
{* See : http://forum.cs-cart.com/tracker/issue-6315-product-code-is-not-visible-on-storefront/?verfilter=127 *}
...
{capture name="discount_label_`$obj_prefix``$obj_id`"}
    {if $show_discount_label && ($product.discount_prc || $product.list_discount_prc) && $show_price_values}
        <span class="ty-discount-label cm-reload-{$obj_prefix}{$obj_id}" id="discount_label_update_{$obj_prefix}{$obj_id}">
            <span class="ty-discount-label__item" id="line_prc_discount_value_{$obj_prefix}{$obj_id}"><span class="ty-discount-label__value" id="prc_discount_value_label_{$obj_prefix}{$obj_id}">{if $smarty.const.CART_LANGUAGE != 'ja'}{__("save_discount")} {/if}{if $product.discount}{$product.discount_prc}{else}{$product.list_discount_prc}{/if}%{if $smarty.const.CART_LANGUAGE == 'ja'}OFF{/if}</span></span>
